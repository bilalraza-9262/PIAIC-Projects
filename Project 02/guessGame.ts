...
let try_again: boolean = true;

//here define interfacxe of inquirer
interface UserInput {
	userInput: number,
	playagain: string
}
...
async function guessNumber() {
	while (try_again) {
		...
		while (chances > 0) {
			...
			const userNumber: UserInput = await inquirer.prompt([{
				name: "userInput",
				type: "number",
		...
		const play_again: UserInput = await inquirer.prompt([{
			name: "playagain",
			type: "input",
