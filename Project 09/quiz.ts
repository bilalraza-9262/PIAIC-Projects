...
//here make all interface
interface UserInput {
  [answer01: string]: string,
  answer02: string,
  answer03: string,
  answer04: string,
  answer05: string,
...
async function Quiz_game() {
  ...
  while (true) {
    ...
    let userAnswer: UserInput = await inquirer.prompt([
      {
        name: "answer01",
        type: "list",
        message: questions[0],
    ...
    for (let i = 0; i < questions.length; i++) {
      const answers: string = userAnswer[`answer0${i + 1}`];
      // 
    ...
    const playAgain: PlayAgain = await inquirer.prompt({
      name: "tryAgain",
      type: "confirm",
